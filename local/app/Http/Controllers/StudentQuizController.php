...
//                     ->limit(10)
        ...
        $record->subject_title = Subject::where('id',$subject_id)->first()->subject_title;
    ...
    public function getDatatable($slug = '')
    {
        ...
        if($slug=='all')
        {
            ...
            ->select(['title', 'dueration', 'category','total_marks','tags','quizzes.slug','quizzes.validity','quizzes.cost' ])
            ->where('total_marks', '!=', 0)
        ...
        $records = Quiz::join('quizcategories', 'quizzes.category_id', '=', 'quizcategories.id')
            ->select(['title', 'dueration', 'category','total_marks','quizzes.slug', 'quizzes.validity','quizzes.cost' ])
        ...
        ->addColumn('action', function ($records) {
            ...
            return '<div class="dropdown more">
                        ...
                        <div class="btn btn-button" aria-labelledby="dLabel">
                            <a onClick="showInstructions(\''.URL_STUDENT_TAKE_EXAM.$records->slug.'\')" href="javascript:void(0);">'.getPhrase("take_exam").'</a>
...
public function userQuizResults()
    {
        ...
        $records = DB::table('users')
                   ...
                   ->select(['quizresults.slug','quizresults.marks_obtained','quizresults.subject_title', 'quizresults.answers','quizresults.subject_analysis','quizresults.percentage','quizresults.exam_status','quizresults.created_at','users.username','users.email','users.phone', 'users.branch', 'users.place', 'users.night_shift', 'users.college'])
                   ->get();
                   return view('admin/quizresults')->with("records", $records, $data, $active_class)->with("data", $data, $active_class);
    }
