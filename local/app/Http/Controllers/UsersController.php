    ...
    */
    ...
    public function getDatatable($slug = '')
    {
        ...
        if($slug=='')
        {
            ...
            ->select(['users.username', 'email', 'phone', 'roles.display_name', 'image' ,'login_enabled','role_id',
            ...
            ->select(['username',  'image', 'email', 'phone', 'roles.display_name','login_enabled','role_id','slug', 'users.updated_at'])
             ->orderBy('users.updated_at', 'desc');
        ...
        ->addColumn('action', function ($records) {
          ...
          $link_data = '<div class="dropdown more">
                        <a id="dLabel" data-toggle="dropdown" class="btn btn-default dropdown-toggle btn-sm" type="button" aria-expanded="true">
                          Action <i class="mdi mdi-chevron-down" style="font-size:15px;"></i>
                        </a>
         ...
         ->editColumn('username', function($records) {
          if(getRoleData($records->role_id)=='student')
            return '<a href="'.URL_USER_DETAILS.$records->slug.'">'.ucfirst($records->username).'</a>';

          return ucfirst($records->username);
        })
