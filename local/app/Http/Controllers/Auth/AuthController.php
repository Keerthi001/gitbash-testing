    ...
    */
    ...
    protected function create(array $data)
    {
       ...
       // $user->image = $data['image'];
        ...
        $user->image = Input::file('image'); // your file upload input field in the form should be named 'file'

        $destinationPath = 'uploads/users/thumbnail';
        // $destinationPath1 = 'uploads/users';
        $filename = $user->image->getClientOriginalName();
        $extension =$user->image->getClientOriginalExtension(); //if you need extension of the file
        $uploadSuccess = Input::file('image')->move($destinationPath, $filename);

        $user->image = $filename;
       // Input::file($destinationPath.$filename)->move($imageObject->getThumbnailSize())->save($destinationPathThumb.$filename);
        ...
        sendEmail('registration', array('user_name'=>$user->name, 'username'=>$data['username'], 'to_email' => $user->email, 'password'=>$data['password'], 'password_confirmation'=>$data['password_confirmation'], 'aadhar_number' => $user->aadhar_number));
    ...
    {
        $login_status = FALSE;
        if (Auth::attempt(['aadhar_number' => $request->email, 'password' => $request->password])) {
